    ...
    <div className={cn("grid gap-6 sm:grid-cols-2 lg:grid-cols-4", className)}>
      {stats.map((stat, index) => (
        <motion.div
          ...
          className="rounded-2xl border border-brand-line/70 bg-white/80 p-6 shadow-sm backdrop-blur"
        >
          <div className="flex items-center gap-3">
            {stat.icon && <div className="text-brand-ink/70">{stat.icon}</div>}
            <dt className="text-xs font-semibold uppercase tracking-[0.3em] text-brand-muted">{stat.label}</dt>
          </div>
